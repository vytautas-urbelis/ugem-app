...
import {GetVoucher} from "../../../axios/axiosBusiness/voucher";
    ...
    const handleQrScan = async (data) => {
        if (data.startsWith("https://ugem.app/")) {
            try {
                ...
                router.dismiss()
                router.push({
                    pathname: `/homeBusiness/scann/onCardScanned/success`,
                    params: {secretKey: getSecretKey.secret_key}
                })
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
            } catch (error) {
                router.dismiss()
                router.push(`/homeBusiness/scann/onCardScanned/failure`)
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
            }

        } else {
            try {
                const response = await GetVoucher(data, token);
                saveCurrentVoucherMMKV(response)
                controlStorage.set("scannedVoucherQr", data);
                router.dismiss()
                router.push(`/homeBusiness/scann/onVoucherScanned/exists`)
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
                console.log('voucheris')
            } catch (error) {
                router.dismiss()
